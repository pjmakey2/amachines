"""
Comando mainline para el plugin de Cobros.
...
class Command(BaseCommand):
    help = 'Registra el plugin de Cobros y sincroniza sus menús'
    ...
    def handle(self, *args, **options):
        ...
        if not register_plugin:
            ...
            self.stdout.write('Uso: python manage.py cobro_mainline --register_plugin')
            return

        if dry_run:
            self.stdout.write(self.style.WARNING('=== DRY RUN - No se harán cambios ===\n'))

        self.stdout.write(self.style.SUCCESS('=== Gestión de Cobros - Mainline ===\n'))

        # 1. Descubrir y registrar el plugin
        self.stdout.write('1. Descubriendo plugin...')
        discovered = plugin_manager.discover_plugins()
        cobro_plugin = next((p for p in discovered if p.name == 'cobro'), None)

        if not cobro_plugin:
            self.stdout.write(self.style.ERROR('Error: Plugin cobro no encontrado'))
            return

        self.stdout.write(f'   Plugin encontrado: {cobro_plugin.display_name} v{cobro_plugin.version}')

        # 2. Registrar en la base de datos
        if not dry_run:
            self.stdout.write('\n2. Registrando plugin en BD...')
            plugin_obj, created = PluginModel.objects.update_or_create(
                name=cobro_plugin.name,
                defaults={
                    'display_name': cobro_plugin.display_name,
                    'description': cobro_plugin.description,
                    'version': cobro_plugin.version,
                    'author': cobro_plugin.author,
                    'app_name': cobro_plugin.app_name,
                    'module_path': cobro_plugin.module_path,
                    'is_core': cobro_plugin.is_core,
                    'dependencies': cobro_plugin.dependencies,
                    'icon': cobro_plugin.icon,
                    'category': cobro_plugin.category,
                }
            )
        ...
        for menu_def in cobro_plugin.menus:
            ...
            if not dry_run:
                menu_obj, created = Menu.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'url': '#',
                        'background': menu_def.get('background', '#10B981'),
                        'active': True,
                    }
                )
                if created:
                ...
                if not dry_run:
                    app_obj, created = Apps.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            'url': item.get('url', ''),
                            'version': '1.0',
                            'background': item.get('background', '#10B981'),
                            'active': True,
                        }
                    )
                    if created:
        ...
        self.stdout.write(self.style.SUCCESS('\nPlugin Gestión de Cobros configurado exitosamente!'))
